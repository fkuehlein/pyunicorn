...
As sections \ref{software_design} and \ref{research_impact_statement} further set out, pyunicorn is now a more versatile and robust tool than ever. The release of v1.0 reflects the package's established API which has largely remained stable since its first publication. Pyunicorn may further serve as an example for the feasibility of the long-term maintenance of open-source scientific software under the typical resource constraints.
